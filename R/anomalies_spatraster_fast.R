anomalies_spatraster_fast <- function(input,
  ...
  sd_idx   <- sqrt(ifel(var_idx < 0, 0, var_idx))
  ...
  if (nlyr(roll_mean_input) > 0) {
    ...
    trend_roll_input  <- .ols_trend_stream(roll_mean_input,  tn_c_roll); names(trend_roll_input)  <- c("roll_input_intercept_t0","roll_input_slope_per_year")
    trend_roll_anom   <- .ols_trend_stream(roll_mean_anom,   tn_c_roll); names(trend_roll_anom)   <- c("roll_anom_intercept_t0","roll_anom_slope_per_year")
    trend_roll_z_anom <- .ols_trend_stream(roll_mean_z_anom, tn_c_roll); names(trend_roll_z_anom) <- c("roll_z_anom_intercept_t0","roll_z_anom_slope_per_year")
  } else {
  ...
  list(
    anom               = anom,               z_anom             = z_anom,
    clim12             = clim12,             sd12               = sd12,
    trend              = trend_stack,        trend_anom         = trend_anom,
    trend_z_anom       = trend_z_anom,       roll_mean_input    = roll_mean_input,
    trend_roll_input   = trend_roll_input,   roll_mean_anom     = roll_mean_anom,
    trend_roll_anom    = trend_roll_anom,    roll_mean_z_anom   = roll_mean_z_anom,
    trend_roll_z_anom  = trend_roll_z_anom,  trend_t0           = as.Date(round(mean(as.numeric(tt), na.rm = TRUE)), origin="1970-01-01"),
