# Faster anomalies + trends + rolling means for terra SpatRaster time stacks
...
anomalies_spatraster <- function(input,
                                 ...
                                 roll_min_obs = NULL,
                                 # performance options (used in tapp calls)
  ...
  .fast_roll_mean <- function(x, k, align = "right", min_obs = ceiling(k/2)) {
    ...
    zero <- cs_x[[1]] * 0  # fast zero raster (same geometry)

    make_diff <- function(cs, s, e) {
    ...
    for (i in seq_along(e)) {
      sum_list[[i]] <- make_diff(cs_x, s[i], e[i])
      n_list[[i]]   <- make_diff(cs_n, s[i], e[i])
    }
    sum_k <- rast(sum_list)
    n_k   <- rast(n_list)
  ...
  # numeric time in years (full & baseline) and centered variants
  ...
  # --- linear trend on RAW over baseline (fast via terra::trend) ---------
  trend_stack <- terra::trend(input_raw[[i_base]], y = tn_c_base, model = "linear")
  ...
  # --- monthly climatology & SD (baseline) -------------------------------
  m_all  <- .get_month(tt)
  m_base <- .get_month(tb)
  ...
  # --- trends over FULL series (fast via terra::trend) --------------------
  trend_anom   <- terra::trend(anom,   y = tn_c_full, model = "linear")
  names(trend_anom) <- c("anom_intercept_t0", "anom_slope_per_year")

  trend_z_anom <- terra::trend(z_anom, y = tn_c_full, model = "linear")
  names(trend_z_anom) <- c("z_anom_intercept_t0", "z_anom_slope_per_year")

  # --- rolling means & their trends (includes z-anomalies) ----------------
  ...
  if (nlyr(roll_mean_input) > 0) {
    # Use any rolling product's timestamps (they should match)
    ...
    trend_roll_input  <- terra::trend(roll_mean_input,  y = tn_c_roll, model = "linear")
    names(trend_roll_input) <- c("roll_input_intercept_t0", "roll_input_slope_per_year")

    trend_roll_anom   <- terra::trend(roll_mean_anom,   y = tn_c_roll, model = "linear")
    names(trend_roll_anom)  <- c("roll_anom_intercept_t0",  "roll_anom_slope_per_year")

    trend_roll_z_anom <- terra::trend(roll_mean_z_anom, y = tn_c_roll, model = "linear")
