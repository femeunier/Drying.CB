...
all.data.long %>%
  filter(station_id == "STN_0001",
         year %in% 2023:2024)

station.selected <- all.data.long %>%
  group_by(station_id,variable) %>%
  summarise(Nhistorical = sum((year %in% c(1961:2010))),
            Nrecent = sum((year %in% c(2023:2024))),
            .groups = "keep") %>%
  filter(Nhistorical > 120,
         Nrecent > 5) %>%
  mutate(station_var = paste0(station_id,"_",variable))

all.data.long <- all.data.long %>%
  mutate(station_var = paste0(station_id,"_",variable)) %>%
  filter(station_var %in% station.selected[["station_var"]])
...
MAP.anomaly <- MAP %>%
  group_by(variable,lon_bin,lat_bin) %>%
  summarise(anomaly = mean(value[year == 2024],
                           na.rm = TRUE) -
              mean(value[year %in% c(1991:2010)],
                   na.rm = TRUE),
            .groups = "keep")

ggplot() +
  geom_raster(data = MAP.anomaly %>%
                filter(variable == "pre") %>%
                na.omit(),
              alpha = 0.8,
              aes(lon_bin, lat_bin,
