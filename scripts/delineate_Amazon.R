...
e <- extent(-100,-30,-25,25)
raw <- crop(raster("~/Downloads/ESA_CCI_pantropical_reclassified_aggregated.tif"),
            e)
plot(raw)

LC <- as.data.frame(raw,xy = TRUE) %>%
  rename(lon = x, lat = y) %>%
...
polys_1 <- polys_sf %>%
  filter(ESA_CCI_pantropical_reclassified_aggregated  == 1)
st_crs(polys_1) <- 4326  # EPSG code for WGS84
st_write(polys_1, "./data/shapefiles/Amazon.shp",append = FALSE)
plot(polys_1)


system2("rsync",
        c("-avz",
          "./data/shapefiles/Amazon*",
